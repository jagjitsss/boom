...
class Trade extends Controller {
	...
	public function getPairData($pair,$decval) 
	{
		...
		if ($pairDetails) 
		{
			...
			$result['buy_orders'] = self::getadvanceBuySellOrders($pairId, 'buy','',$decval);
			$result['limit_buy_orders'] = self::getadvanceBuySellOrders($pairId, 'buy',16,$decval);
			$result['sell_orders'] = self::getadvanceBuySellOrders($pairId, 'sell','',$decval);
			$result['limit_sell_orders'] = self::getadvanceBuySellOrders($pairId, 'sell',15,$decval);
	...
	{
		$coinPairs = array();
		$fav = $favValues = $btcValues = $ethValues = $bnbValues = $ownValues = $usnValues = "";
		...
		$coinPairs['BoomCoin'] = $ownValues;
		$coinPairs['BCH'] = $bnbValues;
