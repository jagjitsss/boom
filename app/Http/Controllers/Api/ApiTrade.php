...
class ApiTrade extends Controller {
	...
	public function getPairDetails() {
		...
		if (isset($data['user_id']) && isset($data['token'])) {
			...
			$pairs = TradePairs::where('status', 1)->select('id', 'min_price', 'max_price', 'trade_fee', 'taker_trade_fee', 'last_price', 'min_amt', 'from_symbol_id', 'to_symbol_id', 'from_symbol', 'to_symbol','convertedeur')->get();
			
			$get_fav = User::where('id', $userId)->select('fav_pairs')->first();
			...
			if ($pairs) {
				foreach ($pairs as $pairDetails) {
					...
					$result['favourite'] = $favour;
					$result['from_symbol'] = $pairDetails->from_symbol;
					$result['from_symbol_name'] = getCurrencyname($pairDetails->from_symbol);
					$result['to_symbol'] = $pairDetails->to_symbol;
					$result['to_symbol_name'] = getCurrencyname($pairDetails->to_symbol);
					$result['min_amount'] = $pairDetails->min_amt;
					$result['min_price'] = $pairDetails->min_price;
					$result['max_price'] = $pairDetails->max_price;
					$result['maker_trade_fee'] = $pairDetails->trade_fee;
					...
					$result['change'] = $trade_data['change'];
					$result['low'] = $trade_data['low'];
					$result['high'] = $trade_data['high'];
					$result['volume'] = $trade_data['volume'];
					$result['convertedeur'] = $trade_data['europrice'];
					...
					if ($userId != 0) {
						...
						$result['from_curbal'] = $getBalance1;
						$result['to_curbal'] = $getBalance2;
	...
	public function getTradeHistory() {
		...
		if (isset($data['pair'])) {
			...
			if ($pairDetails) {
				...
				$result['buy_orders'] = $result['sell_orders'] = $result['open_orders'] = $result['my_orders'] = $result['stop_orders'] = array();
				$result['buy_orders'] = self::getBuySellOrdersApi($pairId, 'buy');
				$result['sell_orders'] = self::getBuySellOrdersApi($pairId, 'sell');
