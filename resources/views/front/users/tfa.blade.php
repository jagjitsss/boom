<div class="return">
    ...
    </a> <p class="titled color-primary">{{trans('app_lang.reset_google') }}</p> <?php echo cms_lang('content','0',session('language'),42);?>
</div>
</div>
...
<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 card-div align-self-stretch d-flex xl-p-r lg-p-r">
	<div class="col-xs-4 col-sm-4 col-md-4 col-lg-4 tfa_blk_one">
		...
		<span class="new_hd">1. {{trans('app_lang.scan_qr') }}</span> 
{!! Form::open(array('id'=>'edittfa','url'=>'updatetfa','method'=>'POST','onsubmit'=>'tfa_load()')) !!}
...
<div class="form-group">
	<label>{{trans('app_lang.key') }}:</label>
	<input type="next" name="" value="{{$secret}}" placeholder="{{$secret}}" readonly>
...
<div class="col-xs-8 col-sm-8 col-md-8 col-lg-8 tfa_blk_two">
		...
		<span class="new_hd">2. {{trans('app_lang.complete_setting') }}</span> 
		
		<div class="form-group">
			<label>{{trans('app_lang.login_password') }}：</label>
			<div class="form_control">
				<input type="password" name="psswd" id="psswd" placeholder="{{trans('app_lang.login_password') }}">
			</div>
			
		</div>
		<div class="form-group">
			<label>{{trans('app_lang.authentic_code') }}：</label>
			<div class="form_control">
				<input type="text" name="onecode" id="onecode" placeholder="{{trans('app_lang.6_digit_code') }}">
			</div>
		</div>
		<div class="form-group">
			<label></label>
			<div class="form_control">
				<button type="submit" class="dsb-blue-btn tfabtn" id="tfabut" disabled="disabled">{{trans('app_lang.confirm') }}</button>
			</div>
		</div>
		{!! Form::close() !!}
		</div>
...
<script type="text/javascript">
	...
	var pass_wrong        = "{{trans('app_lang.pass_wrong') }}";
